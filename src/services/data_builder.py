...
@beartype
def get_repo_activities(github_service: GitHubService, repo_full_name: str, username: Optional[str] = None):
...
def build_user_data(github_service: GitHubService, username: Optional[str] = None):
    ...
    if not activities:
        ...
        for repo in sorted_repos[:5]:
            repo_activities = get_repo_activities(github_service, repo.full_name, username=user.login)
